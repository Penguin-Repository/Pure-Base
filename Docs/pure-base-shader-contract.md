...
- Shader-Core dependency: exactly `jp.lilxyzw.shadercore` `0.1.12`.
...
### PBR and Hybrid direct GGX visibility contract

The direct GGX visibility term is the only direct-BRDF term changed by this contract. PBR and Hybrid share `PureBasePbrEvaluateDirect` through the common PBR model, and both `ForwardBase` and `ForwardAdd` use that same direct evaluator. `ForwardBase` continues to add Unity Standard indirect GI and reflection-probe lighting; `ForwardAdd` remains direct light only.
...
`NdotL` and `NdotV` are the nonnegative direct-light and view cosines. The normal-platform denominator uses `epsilon=1e-5f`; the `SHADER_API_SWITCH` branch uses `UNITY_HALF_MIN`. The helper is written with float parameters and returns a finite visibility expression without square roots. `roughnessFourth=a^2` remains owned by the GGX distribution term and is not derived or consumed by the visibility helper.

The direct evaluator preserves finite ordering by widening the distribution, visibility, and complete direct-specular product to float before the final half result. The specular product remains `distribution * visibility * NdotL * Fresnel`; the unchanged Schlick Fresnel, diffuse response, indirect GI, reflection probes, Meta/lightmapping evaluation, perceptual roughness floor, public property ABI, pass declarations, shader variants, and package dependencies remain outside this visibility-only change. This does not implement Issue #14 multiple-scattering compensation, specular anti-aliasing, or full Unity Standard BRDF parity, and it does not change `_UseUnityStandardDiffuseBrightness`.
...
The fixed characterization domain is `p=[0.089,0.125,0.25,0.5,0.75,1]` and `NdotL/NdotV=[0,0.01,0.05,0.25,0.5,0.75,1]`. The artifact records fast visibility deltas against both the previous `legacy-exact` square-root form and a `regularized-exact` square-root form with the branch-specific additive denominator. Relative differences use `max(abs(reference), 0.0001)` as the near-zero denominator policy.
