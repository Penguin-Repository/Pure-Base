...
namespace PureBase.Tests.Daily
{
    ...
    public sealed class DailyProtectedStateFixture
    {
        ...
        private bool createdTestOwnerScene;

        /// <summary>Prepares the isolated owner-scene fixture and captures protected state before Daily executes.</summary>
        [OneTimeSetUp]
        public void CaptureProtectedState()
        {
            EnsureTestOwnerScene();
            before = ProtectedStateSnapshot.Capture();
        }

        /// <summary>Verifies protected state and removes any owner scene generated for an isolated project.</summary>
        [OneTimeTearDown]
        public void AssertProtectedStateUnchanged()
        {
            ...
            {
                RemoveGeneratedTestOwnerScene();
            }
        }

        /// <summary>Creates the saved non-canonical scene expected by restoration tests when the host project has none.</summary>
        private void EnsureTestOwnerScene()
        {
            if (AssetDatabase.LoadAssetAtPath<SceneAsset>(TestOwnerScenePath) != null)
                return;

            Scene originalActiveScene = SceneManager.GetActiveScene();
            Scene ownerScene = EditorSceneManager.NewScene(
                NewSceneSetup.EmptyScene,
                NewSceneMode.Additive
            );
            try
            {
                if (!EditorSceneManager.SaveScene(ownerScene, TestOwnerScenePath, false))
                {
                    throw new InvalidOperationException(
                        $"Could not create the isolated Daily owner scene '{TestOwnerScenePath}'."
                    );
                }

                createdTestOwnerScene = true;
            }
            finally
            {
            ...
            if (AssetDatabase.LoadAssetAtPath<SceneAsset>(TestOwnerScenePath) == null)
            {
                RemoveGeneratedTestOwnerScene();
                throw new InvalidOperationException(
                    $"The isolated Daily owner scene '{TestOwnerScenePath}' was not imported."
                );
            }
        }

        /// <summary>Deletes only the owner scene generated by this fixture.</summary>
        private void RemoveGeneratedTestOwnerScene()
        {
            if (!createdTestOwnerScene)
                return;
            ...
            if (ownerScene.IsValid() && ownerScene.isLoaded)
            {
                ...
                {
                    Scene replacementScene = EditorSceneManager.NewScene(
                        NewSceneSetup.EmptyScene,
                        NewSceneMode.Additive
                    );
            ...
            bool deleted = AssetDatabase.DeleteAsset(TestOwnerScenePath);
            AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);
            createdTestOwnerScene = false;
            Assert.That(
                deleted,
                Is.True,
                $"Could not delete the generated Daily owner scene '{TestOwnerScenePath}'."
            );
        }
