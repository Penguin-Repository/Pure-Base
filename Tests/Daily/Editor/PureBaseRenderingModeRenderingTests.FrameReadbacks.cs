...
namespace PureBase.Tests.Daily
{
	public sealed partial class PureBaseRenderingModeRenderingTests
	{
		...
		private static Color RenderMetaCenterPixel(Material material)
		{
			...
			try
			{
				return RenderMetaReadback(material, out cameraObject, out quadObject, out renderTexture, out texture, out commandBuffer);
			}
			finally
			{
				globalState.Restore();
				ReleaseMetaReadbackResources(cameraObject, quadObject, renderTexture, texture, commandBuffer);
			}
		}

		/// <summary>Creates and executes the actual Meta-pass command-buffer readback.</summary>
		private static Color RenderMetaReadback(Material material, out GameObject cameraObject, out GameObject quadObject, out RenderTexture renderTexture, out Texture2D texture, out CommandBuffer commandBuffer)
		{
		...
		private static void ReleaseMetaReadbackResources(GameObject cameraObject, GameObject quadObject, RenderTexture renderTexture, Texture2D texture, CommandBuffer commandBuffer)
		{
			Camera camera = cameraObject != null ? cameraObject.GetComponent<Camera>() : null;
