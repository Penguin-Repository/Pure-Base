...
namespace PureBase.Tests.Regeneration
{
    ...
    public sealed class ShaderCoreTestStateInitializerTests
    {
        ...
        public void ConvergeRowsReplacesTargetsPreservesUnrelatedRowsAndBecomesANoOp()
        {
            var actualRows = new[]
            {
                new ShaderCoreTestStateInitializer.ShaderSettingRow("Unrelated/First", new[] { "unrelated.one" }),
                new ShaderCoreTestStateInitializer.ShaderSettingRow("PureBase/Tests/Host", new[] { "stale.module" }),
                new ShaderCoreTestStateInitializer.ShaderSettingRow("PureBase/Tests/Host", new[] { "duplicate.module" }),
                new ShaderCoreTestStateInitializer.ShaderSettingRow("Unrelated/Last", new[] { "unrelated.two", "unrelated.three" })
            };
            ...
            var convergedRows = ShaderCoreTestStateInitializer.ConvergeRows(actualRows, expectedRows, out var changed);

            Assert.That(changed, Is.True);
            Assert.That(convergedRows.Select(row => row.ShaderName), Is.EqualTo(new[] { "Unrelated/First", "PureBase/Tests/Host", "Unrelated/Last", "PureBase/Unlit" }));
            ...
            Assert.That(convergedRows[2].Modules, Is.EqualTo(new[] { "unrelated.two", "unrelated.three" }));
            Assert.That(convergedRows[3].Modules, Is.Empty);

            var secondRunRows = ShaderCoreTestStateInitializer.ConvergeRows(convergedRows, expectedRows, out var secondRunChanged);
        ...
        public void InitializeFailsClosedBeforeStateApplicationWhenSaveContractIsInvalid()
        {
            ...
            {
                new ShaderCoreTestStateInitializer.ShaderSettingRow("Unrelated/Preserved", new[] { "unrelated.module" }),
                new ShaderCoreTestStateInitializer.ShaderSettingRow("PureBase/Tests/Host", new[] { "stale.module" })
            };
            var expectedRows = new Dictionary<string, string[]>
            {
            ...
            var exception = Assert.Throws<InvalidOperationException>(() => ShaderCoreTestStateInitializer.Initialize(
                expectedRows,
        ...
        public void ManifestMapsFixedHostsAndProductShadersToExpectedSelections()
        {
            ...
            Assert.That(expectedRows["PureBase/Tests/ShaderCore/ModuleOrder"], Is.EqualTo(new[]
            {
        ...
        {
            ShaderCoreTestStateInitializer.Initialize();
            var manifest = JsonUtility.FromJson<GeneratedSourceManifest>(File.ReadAllText(ShaderCoreTestStateInitializer.GetManifestPath()));
            ...
            foreach (GeneratedSourceHost host in manifest.hosts)
            {
                ...
                var source = AssetDatabase.LoadAllAssetsAtPath(assetPath).OfType<TextAsset>().SingleOrDefault(asset => asset.name == "Shader Source")?.text;
                Assert.That(source, Is.Not.Null.And.Not.Empty, $"Host '{host.shaderName}' did not produce generated Shader Source.");

                foreach (string sentinel in host.expectedSentinels)
                {
                    AssertPassSentinelCount(source, "ForwardBase", sentinel, host.expectedPassSentinelCounts.ForwardBase, host.shaderName);
                    AssertPassSentinelCount(source, "ForwardAdd", sentinel, host.expectedPassSentinelCounts.ForwardAdd, host.shaderName);
                    AssertPassSentinelCount(source, "ShadowCaster", sentinel, host.expectedPassSentinelCounts.ShadowCaster, host.shaderName);
                    AssertPassSentinelCount(source, "Meta", sentinel, host.expectedPassSentinelCounts.Meta, host.shaderName);
                }

                if (host.shaderName == "PureBase/Tests/ShaderCore/ModuleOrder")
                {
                    AssertModuleOrderSentinelOrder(source, "ForwardBase", host.expectedPassSentinelCounts.ForwardBase, host.shaderName);
                    AssertModuleOrderSentinelOrder(source, "ForwardAdd", host.expectedPassSentinelCounts.ForwardAdd, host.shaderName);
                    AssertModuleOrderSentinelOrder(source, "ShadowCaster", host.expectedPassSentinelCounts.ShadowCaster, host.shaderName);
                    AssertModuleOrderSentinelOrder(source, "Meta", host.expectedPassSentinelCounts.Meta, host.shaderName);
                }

                foreach (string inactiveSentinel in host.inactiveSentinels ?? Array.Empty<string>())
                {
                    Assert.That(CountOccurrences(source, inactiveSentinel), Is.Zero, $"Host '{host.shaderName}' emitted inactive sentinel '{inactiveSentinel}'.");
                }
            }
        }
        ...
        private static string FindHostAssetPath(string shaderName)
        {
            foreach (string guid in AssetDatabase.FindAssets("PureBaseTest", new[] { "Packages/jp.penguin.purebase/Tests/Fixtures/Hosts" }))
            {
                ...
                if (shader != null && shader.name == shaderName) return assetPath;
            }
        ...
        private static void AssertPassSentinelCount(string source, string passName, string sentinel, int expectedCount, string shaderName)
        {
            string passSource = GetPassSource(source, passName, shaderName);
            Assert.That(CountOccurrences(passSource, sentinel), Is.EqualTo(expectedCount), $"Host '{shaderName}' pass '{passName}' emitted unexpected count for '{sentinel}'.");
        }

        /// <summary>Asserts that the generated module-order sentinels retain their configured order in one relevant pass.</summary>
        private static void AssertModuleOrderSentinelOrder(string source, string passName, int expectedCount, string shaderName)
        {
            if (expectedCount == 0) return;

            string passSource = GetPassSource(source, passName, shaderName);
            int zetaIndex = passSource.IndexOf("PUREBASE_TEST_MODULE_ORDER_ZETA", StringComparison.Ordinal);
            int alphaIndex = passSource.IndexOf("PUREBASE_TEST_MODULE_ORDER_ALPHA", StringComparison.Ordinal);
            Assert.That(zetaIndex, Is.GreaterThanOrEqualTo(0), $"Host '{shaderName}' pass '{passName}' did not emit the Zeta module-order sentinel.");
            Assert.That(alphaIndex, Is.GreaterThanOrEqualTo(0), $"Host '{shaderName}' pass '{passName}' did not emit the Alpha module-order sentinel.");
            Assert.That(zetaIndex, Is.LessThan(alphaIndex), $"Host '{shaderName}' pass '{passName}' emitted module-order sentinels out of order: Zeta index {zetaIndex} must be before Alpha index {alphaIndex}.");
        }
        ...
        private sealed class InvalidSaveMethodReflectionContractResolver : ShaderCoreTestStateInitializer.IReflectionContractResolver
        {
            /// <inheritdoc />
        ...
        private sealed class RecordingStateApplicationFactory : ShaderCoreTestStateInitializer.IStateApplicationFactory
        {
            ...
            public RecordingStateApplicationFactory(ShaderCoreTestStateInitializer.IStateApplication stateApplication)
            {
                this.stateApplication = stateApplication ?? throw new ArgumentNullException(nameof(stateApplication));
            }
            ...
            /// <inheritdoc />
            public ShaderCoreTestStateInitializer.IStateApplication Create(ShaderCoreTestStateInitializer.ProjectSettingsReflectionContract reflectionContract)
            {
                CreateCallCount++;
        ...
        private sealed class RecordingStateApplication : ShaderCoreTestStateInitializer.IStateApplication
        {
            ...
            public RecordingStateApplication(IEnumerable<ShaderCoreTestStateInitializer.ShaderSettingRow> rows)
            {
                this.rows = new List<ShaderCoreTestStateInitializer.ShaderSettingRow>(rows ?? throw new ArgumentNullException(nameof(rows)));
            }
            ...
            /// <inheritdoc />
            public void WriteRows(IReadOnlyList<ShaderCoreTestStateInitializer.ShaderSettingRow> newRows)
            {
                WriteRowsCallCount++;
                rows.Clear();
