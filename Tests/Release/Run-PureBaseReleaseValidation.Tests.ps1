...
'@
        ...
        try {
            ...
            foreach ($successfulLabel in @('module-free-clean-import', 'progressive-cpu-bake')) {
                ...
                Assert-Harness -Condition ($initializationReport.schemaName -eq 'purebase-shader-core-bootstrap-initialization' -and $initializationReport.phase -eq 'first-bootstrap' -and $initializationReport.canonicalConfigDestination -eq $canonicalConfigDestination -and $initializationReport.rowCount -eq 17) -Message "Successful row '$successfulLabel' did not report the canonical first Shader-Core initialization mapping."
                Assert-Harness -Condition (@($initializationReport.mapping.modules.'PureBase/Tests/ShaderCore/ToonShadow').Count -eq 1 -and $initializationReport.mapping.modules.'PureBase/Tests/ShaderCore/ToonShadow'[0] -eq 'jp.penguin.purebase.tests.shadercore.toonshadow' -and @($initializationReport.mapping.modules.'PureBase/Tests/ShaderCore/ToonOpenLitGamma').Count -eq 1 -and $initializationReport.mapping.modules.'PureBase/Tests/ShaderCore/ToonOpenLitGamma'[0] -eq 'jp.penguin.purebase.tests.shadercore.toonopenlitgamma') -Message "Successful row '$successfulLabel' did not report the exact canonical Toon Shader-Core mappings during first bootstrap."
                ...
                Assert-Harness -Condition ($secondInitializationReport.phase -eq 'second-bootstrap' -and $secondInitializationReport.rowCount -eq 17) -Message "Successful row '$successfulLabel' did not revalidate the canonical Shader-Core state after the Library reset."
                Assert-Harness -Condition (@($secondInitializationReport.mapping.modules.'PureBase/Tests/ShaderCore/ToonShadow').Count -eq 1 -and $secondInitializationReport.mapping.modules.'PureBase/Tests/ShaderCore/ToonShadow'[0] -eq 'jp.penguin.purebase.tests.shadercore.toonshadow' -and @($secondInitializationReport.mapping.modules.'PureBase/Tests/ShaderCore/ToonOpenLitGamma').Count -eq 1 -and $secondInitializationReport.mapping.modules.'PureBase/Tests/ShaderCore/ToonOpenLitGamma'[0] -eq 'jp.penguin.purebase.tests.shadercore.toonopenlitgamma') -Message "Successful row '$successfulLabel' did not report the exact canonical Toon Shader-Core mappings during second bootstrap."
