...
function New-ProductContract {
    ...
    if ($ShaderName -eq 'PureBase/PBR' -or $ShaderName -eq 'PureBase/Hybrid') {
        ...
        $requiredSourceFragments += '[SCRange(0.089,1)] _Roughness ("Roughness", Float) = 0.5'
    }
    return [ordered]@{
...
try {
    ...
    if (-not $ModuleFreeOnly) {
        ...
        if (-not $ToonBaseOnly -and -not $CompareWarmAndColdStandardMorph) {
            ...
            $postPixelAlphaModule = [ordered]@{ label = 'rendering-mode-postpixel-alpha'; phase = 'postpixel'; uniqueId = 'jp.penguin.purebase.release.fixture.products.postpixel'; propertyName = ''; sentinel = '' }
